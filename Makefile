...
# App information
APP_TITLE           :=	PP2D-Test
APP_DESCRIPTION     :=	PP2D-Test
APP_AUTHOR          :=	T3CHNOLOG1C
...
release: all
	...
	@zip -r $(APP_TITLE)-$(VERSION_MAJOR).$(VERSION_MINOR).$(VERSION_MACRO).zip 3ds/ $(APP_TITLE).cia
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(OUTDIR)
	@rm -rf $(APP_TITLE)-$(VERSION_MAJOR).$(VERSION_MINOR).$(VERSION_MACRO).zip
	@rm -rf $(APP_TITLE).cia
	@rm -rf 3ds/
#---------------------------------------------------------------------------------
ifeq ($(strip $(NO_SMDH)),)
